...
plot(reg1_gam_f)
```
For wheat, both predictors are statistically significant: the effect of potential yield change is nonlinear (edf = 5.6, p < 0.001), while historical yield shows a strong linear association (edf = 1.0, p < 0.001), suggesting that communes with higher past performance tended to expand wheat surfaces. 
In the case of maize, only historical yield is significant (p = 0.025), though with a weaker effect, and there is no meaningful relationship with the change in potential yield (p = 0.12). 
For pea, neither variable reaches statistical significance. 
Finally, for flax, the change in potential yield has a highly significant and nonlinear effect (p < 0.001, edf = 7.3), while historical yield is only marginally significant (p ≈ 0.095). 
...
GAEZ_France_th3 <- GAEZ_France |>
  filter(theme_id == 3)

GAEZ_France_th3 <- GAEZ_France_th3 |>
  filter(!crop %in% c(
    "Biomass sorghum",
    "Cassava",
    "Coconut",
...
GAEZ_France_th3_match <- GAEZ_France_th3 |>
  left_join(correspondance_gaez_rpg, by = c("crop" = "culture_gaez"))

# create weights associated to each GAEZ crop
GAEZ_France_th3_weight <- GAEZ_France_th3_match |>
  group_by(insee, groupe_rpg) |>
  mutate(
    sum_value_hist_group = sum(value_hist, na.rm = TRUE),
    weight = if_else(sum_value_hist_group > 0,
                     value_hist / sum_value_hist_group,
                     0)
  ) |>
  ungroup()

GAEZ_Legumineuses <- GAEZ_France_th3_weight |>
...
rpg_level <- GAEZ_France_th3_weight |>
  group_by(insee,groupe_rpg) |>
  summarise(
...
RPG_cultures_transf <- RPG_cultures |>
  mutate(LIBELLE_GROUPE_CULTURE = case_when(
  ...
  filter(!LIBELLE_GROUPE_CULTURE %in% c("Vignes", "Fruits à coque", "Gel (surfaces gelées sans production)", NA))
...
fusion_cultures <- c("Pâturages", "Légumineuses à grains/Protéagineux")
