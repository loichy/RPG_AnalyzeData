...
df_final_etat_NA$etat <- as.factor(df_final_etat_NA$etat)
...
logit_1 <- multinom(etat ~ value_hist_rpg + change, data = df_final_etat_NA)
summary(logit_1)
...
logit_2 <- multinom(etat ~ AC_hist + AC_diff, data = df_final_etat_NA)
summary(logit_2)
...
logit_3 <- multinom(etat ~ AC_hist + change, data = df_final_etat_NA)
summary(logit_3)
...
logit_c1 <- multinom(etat ~ value_hist_rpg + change + moyenne_temp_period + moyenne_temp_ref, data = df_final_etat_NA)

logit_c2 <- multinom(etat ~ AC_hist + AC_diff + moyenne_temp_period + moyenne_temp_ref, data = df_final_etat_NA)

logit_c3 <- multinom(etat ~ AC_hist + change + moyenne_temp_period + moyenne_temp_ref, data = df_final_etat_NA)
...
RPG_panel <- RPG_panel |>
  ...
  select(insee, year, region_code, LIBELLE_GROUPE_CULTURE_AGG, surf_tot_geo_unit_m2, surf_agri_geo_unit_m2, surf_code_group_perc, surf_code_group_m2, mean, reference_mean) |>
  arrange(insee, year, LIBELLE_GROUPE_CULTURE_AGG)


## will add precipitations dataset when downloaded
...
df_panel_wheat <- df_panel |>
  ...
  mutate(log_surf_agri = log(surf_agri_geo_unit_m2),
         change = (value_futur_rpg - value_hist_rpg) / value_hist_rpg)

baseline_model_w <- lm(drop_units(log_surf_agri) ~ mean + value_hist_rpg + reference_mean, data = df_panel_wheat)
summary(baseline_model_w)

interact_model_w <- lm(drop_units(log_surf_agri) ~ mean*change + value_hist_rpg + reference_mean, data = df_panel_wheat)
summary(interact_model_w)
...
baseline_model_m <- lm(drop_units(log_surf_agri) ~ mean + value_hist_rpg + reference_mean, data = df_panel_maize)
summary(baseline_model_m)

interact_model_m <- lm(drop_units(log_surf_agri) ~ mean*change + value_hist_rpg + reference_mean, data = df_panel_maize)
summary(interact_model_m)

models_cui <- list(
  "Wheat (1)" = baseline_model_w,
  "Maize (1)" = baseline_model_m,
  "Wheat (2)" = interact_model_w,
  "Maize (2)" = interact_model_m
)
...
```
We first notice that across all models, historical yield has a coefficient close to 0 despite it being highly significant. We can suppose that the significance of this variable is largely driven by sample size, as we cannot interpret any magnitude of effect of the variable on acreage size. 

In the baseline models, we observe that mean temperature has a highly significant and negative correlation with both **Wheat** and **Maize** acreage size. This would mean that an increase in yearly mean temperature leads to a decrease in acreage size. It is interesting to note directly that the inverse effect comes from the reference mean temperature, which corresponds to the mean temperature level in the reference period between 1971 and 2000. 

In the second series of models, we add potential yield change as well as an interaction term between mean temperature level and the former variable. The idea is to show that the effect of potential yield change on the outcome could be influenced by observed temperature, or vice versa. For wheat, we find that potential yield change alone is negatively correlated with wheat surface size, while positively correlated with that of maize. The contrary signs are found on the coefficients of the interaction term. We can however note that the coefficient on the interaction term where the outcome is acreage dedicated to maize is close to 14 times smaller than that on potential yield change, despite it being highly significant. 

We can note also that the adjusted R² consistently improve when going from the baseline to the second specification. 
